...
your system, you can use [gettimeofday](http://man7.org/linux/man-pages/man2/gettimeofday.2.html) + nanosleep, but you'll lose a precision and the monotony because
of the gettimeofday. You should test it first and maybe it will be acceptable
in your case.
...
of the Mach kernel (Oh, Holy Moly).
...
A right consists of a port name and a capability—send or receive—on that port,
...
// longer need it.
//
// You should be very careful with mach ports allocation and deallocation,
...
For more details about Mach API, use following resources:
...
There are [benchmarks](LINK) to demonstrate it.

See also: [libuv fight with OS X clock API](https://github.com/joyent/libuv/pull/1325)
...
It is extremly fast, because it uses [RDTSC](https://en.wikipedia.org/wiki/Time_Stamp_Counter)
underneath. I hope, that XNU synchronizes this value between multiple cores.
